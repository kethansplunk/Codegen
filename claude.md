...
- `scripts/migrate_sql_to_nosql.py` — Phase 20C, migrates one Spider SQL database to MongoDB via `MongoDBConverter` (5B), verified end-to-end against a real local `mongod`
...
- Phase 20C: SQL-to-NoSQL migration utility (`scripts/migrate_sql_to_nosql.py`) — thin CLI over `MongoDBConverter.convert_database()` (Phase 5B), no new conversion logic. Verified for real against a local `mongod` (`concert_singer` → 31 docs / 4 collections, confirmed via direct `pymongo` query). 3 tests against a stub converter.

## Testing
`pytest tests/` — 78 tests, all run on Mac with no GPU and no checkpoints (SchemaLinker/SAR/Generator stubbed; real LangGraph graph, real POSG, real temp SQLite DBs, real regex-based SQL fixup logic). Needs `pytest`, `sqlparse`, `langgraph`, `fastapi` installed.
